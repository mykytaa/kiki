...
import time, sqlite3, json, math, os
...
# Вставил твой токен по умолчанию (можешь переопределить в UI)
DEFAULT_TG_TOKEN = "6231361993:AAFCKT2rPnoJv2K4OXAZCpOq8KcjmFGvJjw"
...
def slope_series(s: pd.Series, last_n=80) -> float:
    n = min(len(s), last_n)
    if n < 8:
        return 0.0
    y = s.tail(n).values
...
def strict_fvg_validate(direction, gaps, *, price, vp, atr_val, bos_time,
...
def db_init():
    ...
    if "reasons" not in cols:       cur.execute("ALTER TABLE trades ADD COLUMN reasons TEXT")
    if "journal_state" not in cols: cur.execute("ALTER TABLE trades ADD COLUMN journal_state TEXT")
    if "result" not in cols:        cur.execute("ALTER TABLE trades ADD COLUMN result TEXT")
    if "result_ts" not in cols:     cur.execute("ALTER TABLE trades ADD COLUMN result_ts TEXT")
...
def db_upsert_trade(row:dict):
    con=sqlite3.connect(DB); cur=con.cursor()
    cur.execute("""
        INSERT INTO trades(ts,asset,tf,name,bias,entry,sl,tp1,tp2,confirms,reasons,status,journal_state,result,result_ts)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, (
        row["ts"],row["asset"],row["tf"],row["name"],row["bias"],
        row["entry"],row["sl"],row.get("tp1"),row.get("tp2"),
        row["confirms"], json.dumps(row["reasons"],ensure_ascii=False),
...
def db_list_trades(limit=300):
    con=sqlite3.connect(DB); cur=con.cursor()
    cur.execute("""
        SELECT id,ts,asset,tf,name,bias,entry,sl,tp1,tp2,confirms,reasons,status,journal_state,result,result_ts
...
def rr_targets(entry, sl, bias, min_rr=2.0):
    ...
    tp1 = entry + min_rr*risk if bias=="long" else entry - min_rr*risk
    tp2 = entry + 3.0*risk if bias=="long" else entry - 3.0*risk
...
def propose(df, rules, htf_bias, d_bias, regime, vp, asset, tf) -> List[Scenario]:
    ...
    ema20=float(ema(df.close,20).iloc[-1]); vw=float(vwap_series(df).iloc[-1])
    ...
    def gen_conf(bias, entry):
        ...
        poc_ok = (price>vp["poc"] and bias=="long") or (price<vp["poc"] and bias=="short")
        if poc_ok: out.append("сторона POC")
    ...
    if (swp["low"] and dir_bos=="up") or (swp["high"] and dir_bos=="down"):
        ...
        if fvg_ok:
            ...
            add(
                name="SFP→BOS→FVG", bias=direction, etype="limit",
                trigger=f"касание mid FVG {mid:.5f}",
                entry=float(entry), sl=float(sl), tp1=float(tp1), tp2=float(tp2), rr=rr,
                confirms=len(set(confirms)), confirm_list=confirms,
                explain_short="срыв→пробой→имбаланс (0.5)",
                stop_reason="за SFP ±0.7×ATR", tp_reason="структурная цель/POC",
                logic_path=["SFP","BOS","FVG"], status=status, missing=missing
            )

    # 2) BOS → OB Retest (+ iFVG / BPR одно из)
    def ob_block(dir_):
    ...
    if dir_bos=="up":
        ...
        if ob:
            ...
            add(
                ...
                trigger=f"OB {lo:.5f}-{hi:.5f}",
                entry=float(entry), sl=float(sl), tp1=float(tp1), tp2=float(tp2), rr=rr,
                confirms=len(set(confirms)), confirm_list=confirms,
                explain_short="пробой и ретест спроса (+iFVG/BPR)",
    ...
    if dir_bos=="down":
        ...
        if ob:
            ...
            add(
                ...
                entry=float(entry), sl=float(sl), tp1=float(tp1), tp2=float(tp2), rr=rr,
                confirms=len(set(confirms)), confirm_list=confirms,
                explain_short="пробой и ретест предложения (+iFVG/BPR)",
                stop_reason="за OB +0.6×ATR", tp_reason="структурная цель/POC",
                logic_path=["BOS","OB","iFVG/BPR"], status=status, missing=missing
            )
    ...
    if swp["high"] and dir_bos=="down":
        ...
        status="ok" if len(set(base))>=3 else "await"
        add(
            name="Breaker", bias="short", etype="stop",
            trigger=f"возврат под {lv:.5f}",
            entry=float(entry), sl=float(sl), tp1=float(tp1), tp2=float(tp2), rr=rr,
            confirms=len(set(confirms)), confirm_list=confirms,
            explain_short="срыв high и возврат",
            stop_reason="над свип-уровнем +0.7×ATR", tp_reason="структурная цель/POC",
            logic_path=["SFP","BOS","ret"], status=status, missing=[]
        )
    if swp["low"] and dir_bos=="up":
        ...
        status="ok" if len(set(base))>=3 else "await"
        add(
            name="Breaker", bias="long", etype="stop",
            trigger=f"возврат над {lv:.5f}",
            entry=float(entry), sl=float(sl), tp1=float(tp1), tp2=float(tp2), rr=rr,
            confirms=len(set(confirms)), confirm_list=confirms,
            explain_short="срыв low и возврат",
    ...
    if regime=="range" and float(adx(df).iloc[-1])<22:
        if abs(price-vp["val"])<=max(0.6*at,0.1*(vp["vah"]-vp["val"])):
            ...
            base=["VAL edge","range","POC target"]; generic=gen_conf("long", entry)
            confirms=base+generic; status="ok" if len(set(base))>=3 else "await"
            add(
                name="Value Area Reversion", bias="long", etype="limit",
                trigger="от VAL к POC",
                entry=float(entry), sl=float(sl), tp1=float(tp1), tp2=float(tp2), rr=rr,
                confirms=len(set(confirms)), confirm_list=confirms,
                explain_short="от края к POC",
                stop_reason="под VAL −0.8×ATR", tp_reason="POC",
                logic_path=["VAL","range","POC"], status=status, missing=[]
            )
        if abs(price-vp["vah"])<=max(0.6*at,0.1*(vp["vah"]-vp["val"])):
            ...
            base=["VAH edge","range","POC target"]; generic=gen_conf("short", entry)
            confirms=base+generic; status="ok" if len(set(base))>=3 else "await"
            add(
                name="Value Area Reversion", bias="short", etype="limit",
                trigger="от VAH к POC",
                entry=float(entry), sl=float(sl), tp1=float(tp1), tp2=float(tp2), rr=rr,
                confirms=len(set(confirms)), confirm_list=confirms,
                explain_short="от края к POC",
                stop_reason="над VAH +0.8×ATR", tp_reason="POC",
                logic_path=["VAH","range","POC"], status=status, missing=[]
            )
    ...
    def _k(s:Scenario):
        ...
        if (s.name.startswith(("SFP→BOS→FVG","BOS→OB","Breaker")) and regime=="trend") or \
           (s.name.startswith("Value Area") and regime=="range"):
            sc+=1
...
def yf_ohlc_first_success(asset_key, tf, limit=800):
    ...
    raise RuntimeError(f"{asset_key}: не удалось получить свечи для выбранного TF. Подробности: {last_err}")
...
with tab_signals:
    ...
    with colC: min_risk_pct = st.number_input("Мин. риск (%ATR)", 5, 60, value=25, step=5)
    with colD: min_tp1_atr = st.number_input("Мин. TP1 (×ATR)", 1.0, 3.0, value=1.5, step=0.25)
    with colE: min_confirms = st.number_input("Мин. подтверждений", 2, 5, value=rules.get("min_confirms",3), step=1)
    colF,colG,colH = st.columns([1,1,1])
    with colF: refresh_mode = st.selectbox("Обновление", ["Выключено","30s","1m","2m","5m"], index=0)
    with colG: tg_token = st.text_input("Telegram bot token", value=DEFAULT_TG_TOKEN, type="password")
    ...
    if refresh_mode!="Выключено":
        ...
        if now>=st.session_state.next_refresh_ts:
            st.session_state.next_refresh_ts=now+it
            st.cache_data.clear()
            st.experimental_rerun()
    else:
        st.session_state.next_refresh_ts=time.time()+10**9
    ...
    try:
        ...
        for s in scen_all:
            ...
            if risk_ok and tp1_ok:
                if s.confirms>=min_confirms and s.status=="ok":
                    scen.append(s)
                else:
                    awaiting.append(s)
        ...
        st.markdown(
            f"**Контекст:** LTF={score_bias(df).upper()}, HTF={htf_bias.upper()}, Daily={d_bias.upper()} • "
        ...
        for i, s in enumerate(scen, 1):
            key=f"{s.name} ({s.bias})"
            state_txt = ("OK" if (s.confirms>=3 and s.status=="ok") else "2/3 ждём")
            cols = st.columns([3,1,1,1,1,1.2,1.2,1.2])
            with cols[0]:
                st.write(f"**{key}** — {state_txt}")
            ...
            with cols[5]:
                if st.button("В журнал: лимитка", key=f"j_addL_{i}"):
                    db_upsert_trade({
                        "ts": str(df.index[-1]), "asset": asset, "tf": tf,
                        "name": s.name, "bias": s.bias,
                        "entry": s.entry, "sl": s.sl, "tp1": s.tp1, "tp2": s.tp2,
                        "confirms": s.confirms, "reasons": s.confirm_list,
                        "status": s.status, "journal_state": "limit"
                    })
            ...
            with cols[6]:
                if st.button("В журнал: в рынке", key=f"j_addM_{i}"):
                    db_upsert_trade({
                        ...
                        "entry": s.entry, "sl": s.sl, "tp1": s.tp1, "tp2": s.tp2,
                        "confirms": s.confirms, "reasons": s.confirm_list,
                        "status": s.status, "journal_state": "in_position"
                    })
        ...
        if tg_token and tg_chat and scen:
            for s in scen:
                ...
                try:
                    text = (
                        f"{asset} {tf}: {s.name} {s.bias} — "
                        f"{'ГОТОВ 3/3' if level=='3of3' else '2/3, ждём 3'}\n"
        ...
        # дружелюбное описание для Yahoo ограничений
        msg = str(e)
        if "пусто" in msg or "нет данных" in msg:
            st.error(
                f"{asset}: не удалось получить свечи для выбранного TF. "
                f"У Yahoo интрадей (≤60m) доступен только за ~60 дней. Подробности: {msg}"
            )
        else:
            st.error(f"{asset}: {e}")
...
with tab_journal:
    ...
    if not jrows:
        ...
        for r in jrows:
            ...
            with c1:
                st.write(
                    f"#{r['id']} {r['ts']} • {r['asset']} {r['tf']} • {r['name']} {r['bias']} • "
                    f"entry {r['entry']:.6f} sl {r['sl']:.6f} tp1 {r['tp1']:.6f}"
                )
            with c2:
                if st.button("TP", key=f"tp_{r['id']}"):
                    db_update_trade_status(r['id'], result="tp")
                    st.experimental_rerun()
            with c3:
                if st.button("SL", key=f"sl_{r['id']}"):
                    db_update_trade_status(r['id'], result="sl")
                    st.experimental_rerun()
            with c4:
                if st.button("Отмена", key=f"cn_{r['id']}"):
                    db_update_trade_status(r['id'], result="cancel")
                    st.experimental_rerun()
            with c5:
                if st.button("Закрыть рыночн.", key=f"mk_{r['id']}"):
                    db_update_trade_status(r['id'], result="mkt_close")
                    st.experimental_rerun()
...
with tab_rules:
    ...
    need_side_chk = st.checkbox("Требовать сторону POC", value=rw["need_poc_side"])
    rw["need_poc_side"]=need_side_chk
    st.write("Веса (доп. бонусы):")
    cw=rw["weights"]
    cw["obv_align"]=st.number_input("OBV в сторону",0.0,1.5,float(cw["obv_align"]),step=0.1)
    cw["ema_trend"]=st.number_input("Тренд EMA20",0.0,1.5,float(cw["ema_trend"]),step=0.1)
    cw["poc_side"]=st.number_input("Сторона POC",0.0,1.5,float(cw["poc_side"]),step=0.1)
    cw["vwap_near"]=st.number_input("Рядом VWAP",0.0,1.5,float(cw["vwap_near"]),step=0.1)
    cw["divergence"]=st.number_input("RSI дивергенция",0.0,1.5,float(cw["divergence"]),step=0.1)
    rw["min_confirms"]=st.number_input("Мин. подтверждений по умолчанию",2,5,int(rw.get("min_confirms",3)),step=1)
    if st.button("💾 Сохранить правила"):
        save_rules(rw)
        st.success("Сохранено. Обнови вкладку Сигналы при необходимости.")

...
POC flip, Range reversion. Вероятности основаны на конфлюэнс-оценке.
Beginner-карточка + Справочник.
"""
...
HTF_MAP = {"5m": "15m", "15m": "1h", "1h": "4h"}  # "4h" эмулируем через 60m+агрегацию при необходимости
...
def swings(df: pd.DataFrame, L=3, R=3):
    ...
    for i in range(L, n - R):
        if hi[i] == hi[i - L : i + R + 1].max():
            SH[i] = True
        if lo[i] == lo[i - L : i + R + 1].min():
            SL[i] = True
...
# генератор разного текста (без повторяющейся заглушки)
def explain_scenario(name: str, bias: str, details: Dict[str, float | str]) -> str:
    ...
    if name == "FVG mitigation":
        parts.append("Откат к середине последнего FVG по направлению подтверждённого BOS.")
        parts.append(f"Подтверждение: импульс в сторону {side}, закрытие за swing-уровнем, затем возврат в FVG.")
    elif name == "OB Retest":
        parts.append("Ретест импульсного Order Block по направлению движения.")
        parts.append("Подтверждение: тест границы блока с откатом; слабые хвосты внутри блока.")
    elif name == "BOS Break & Retest":
        lvl = details.get("lvl")
        parts.append(f"Пробой ключевого swing-уровня {lvl:.2f} и ретест в его зону.")
        parts.append("Подтверждение: свеча закрепляется за уровнем, ретест даёт отбой по направлению пробоя.")
    elif name == "Breaker":
        lvl = details.get("lvl")
        parts.append(f"Свип ликвидности у уровня {lvl:.2f} и возврат (breaker).")
        parts.append("Подтверждение: ложный прокол и уверенное закрытие обратно.")
    elif name == "Value Area Reversion":
        edge = details.get("edge")
        parts.append(f"От края value area ({edge}) к POC (mean-reversion).")
        parts.append("Подтверждение: разворотная свеча у VAL/VAH; ADX низкий, рынок в боковике.")
    elif name == "EMA Pullback":
        parts.append("Продолжение тренда через откат к EMA20.")
        parts.append("Подтверждение: касание EMA и возобновление импульса в сторону тренда.")
    elif name == "Structure Breakout":
        lvl = details.get("lvl")
        parts.append(f"Пробой последнего фрактального уровня {lvl:.2f} и продолжение хода.")
        parts.append("Подтверждение: ускорение и отсутствие быстрых откатов после пробоя.")
    elif name == "VWAP Bounce":
        parts.append("Работа от VWAP: откат к VWAP с отбитием в сторону баланса объёма.")
        parts.append("Подтверждение: свеча удерживает VWAP с телом в сторону входа; OBV согласован.")
    elif name == "POC Flip":
        parts.append("Перехват контроля у POC: ретест POC с обратной стороны.")
        parts.append("Подтверждение: POC выступает поддержкой/сопротивлением после пробоя.")
    elif name == "SFP Reversal":
        lvl = details.get("lvl")
        parts.append(f"SFP у фрактала {lvl:.2f}: прокол и закрытие обратно, ожидается реверс.")
        parts.append("Подтверждение: медвежья/бычья свеча после прокола; хвост длиннее тела.")
    else:
        parts.append("Сетап по контексту и уровням.")
    parts.append("Отмена: сильная свеча против входа и закрепление за уровнем стопа.")
    return " ".join(parts)
...
def propose(df: pd.DataFrame, htf_bias: str, d_bias: str, regime: str,
    ...
    gaps = fvg(df)
    pools = liquidity_pools(df, SH, SL)
    ob = simple_ob(df, dir_, t)
    sh_lvl, sl_lvl = last_swing_levels(df, SH, SL)
    near_bsl = sorted([p for _, p in pools["BSL"]], key=lambda x: abs(x - price))[:2]
    near_ssl = sorted([p for _, p in pools["SSL"]], key=lambda x: abs(x - price))[:2]
    ...
    # --- Structure Breakout (оба направления при наличии уровней)
    ...
    if abs(price - vp["poc"]) <= 0.6 * at:
        if price > vp["poc"]:
            ...
            tp1, tp2, rr = rr_targets(e, sl, "long")
            sc.append(Scenario("POC Flip", "long", "limit", "ретест POC сверху вниз", e, sl, tp1, tp2, rr,
                               explain_scenario("POC Flip", "long", {})))
        else:
            e = vp["poc"] - 0.1 * at; sl = vp["poc"] + 0.7 * at
            tp1, tp2, rr = rr_targets(e, sl, "short")
            sc.append(Scenario("POC Flip", "short", "limit", "ретест POC снизу вверх", e, sl, tp1, tp2, rr,
                               explain_scenario("POC Flip", "short", {})))

    # --- SFP (swing failure) на последнем swing
...
def scenario_probabilities(
    ...
    *,
    ...
    scores = []
    labels = []
    vw = vp.get("poc", price)  # используем POC как "центр" если нет VWAP тут
    for s in scen:
        ...
        # режим рынка
        if (s.name.startswith(TREND_TYPES) and regime == "trend") or (s.name.startswith(RANGE_TYPES) and regime == "range"):
            sc += 1.0
        if regime == "range" and s.name.startswith("Structure"):
            sc -= 0.6
        # расстояние до входа
        ...
        if dist > 2.0:
            ...
            sc -= 0.5
        # позиционирование относительно POC/VWAP
...
def readable_analysis(symbol: str, tf: str,
    ...
    ltf_bias = score_bias(df_ltf)
    htf_bias = score_bias(df_htf)
    ...
    lines.append(f"• OBV: {'растёт' if slope > 0 else 'падает' if slope < 0 else 'плоский'}; VP: POC {vp['poc']:.2f}, VAL {vp['val']:.2f}, VAH {vp['vah']:.2f} → цена {poc_state}.")
    lines.append(f"**Самый вероятный:** {top[0]} ≈ {top[1]:.1f}%")
    lines.append(f"**Баланс:** LONG {bias_summary['long']:.1f}% vs SHORT {bias_summary['short']:.1f}%")
    return "\n".join(lines)
...
def make_beginner_summary(symbol: str, tf: str, price: float,
    ...
    where_price = (
        "выше диапазона (VAH)" if price > vp["vah"]
...
def render_beginner_card(st, sc: Scenario, prob: float, atr_val: float):
    ...
    st.markdown(
        ...
        "- **Не входить, если:** импульс ушёл далеко от точки, стоп становится чрезмерным, "
...
def binance_klines(symbol: str, interval: str, limit: int = 800) -> pd.DataFrame:
    ...
    return df[need].tail(limit)

# ======== Guide ========
def show_guide():
    st.markdown("## Справочник: термины, модели, как пользоваться")
    st.markdown(
        "- **ATR** — средний истинный диапазон. Для стопов, подтверждений, фильтров.\n"
        "- **Фракталы (swings)** — локальные экстремумы, по ним строятся уровни и BOS.\n"
        "- **BOS** — break of structure, подтверждаем закрытием за уровнем ≥ 0.30×ATR.\n"
        "- **FVG (Fair Value Gap)** — дисбаланс между свечами. Берём откат в середину по тренду.\n"
        "- **OB (Order Block)** — импульсная свеча противоположного цвета. Ретест даёт вход.\n"
        "- **Breaker** — свип фрактала и возврат под/над уровень.\n"
        "- **Value Area / POC** — объёмная область и её центр. В боковике играем от VAL/VAH к POC.\n"
        "- **VWAP** — средневзвешенная цена по объёму; хорош для откатов к балансу.\n"
        "- **SFP** — swing failure pattern (прокол и возврат), чаще даёт реверс.\n"
        "- **EV** — ожидаемая ценность: полезно для сравнения сценариев.\n"
    )
    st.markdown("### Как пользоваться")
    st.markdown(
        "1) Выбери **TF** (по умолчанию 5m) и автообновление.\n"
        "2) Выставь фильтры **Мин. риск (%ATR)** и **Мин. TP1 (×ATR)**, чтобы отсечь шум.\n"
        "3) Включи **Простой режим**, если хочешь короткую конкретику.\n"
        "4) Смотри таблицу сценариев и блок EV, чтобы выбрать лучший по вероятности и ожиданию.\n"
        "5) Уважай режим: в боковике — mean-reversion, в тренде — продолжение.\n"
    )
    st.info("Вероятности — относительные (softmax), это инструмент ранжирования идей, а не гарантия результата.")
...
st.caption(
    "Сетапы с риском ниже порога %ATR и с TP1 меньше заданного множителя ATR скрываются. "
...
for s in SYMBOLS:
    try:
        ...
        htf = HTF_MAP[tf]
        htf_interval = BINANCE_INTERVAL.get(htf, "1h")
        df_h = binance_klines(s, htf_interval, limit=HTF_LIMIT)
        df_d = binance_klines(s, "1h", limit=24 * 200)  # эмуль Daily из 1h (yfinance 1d бывает грубее); хватит для bias
        ...
        if beginner_mode:
            st.markdown(
                make_beginner_summary(
                    s, tf, price, reg, ltf_b, htf_b, d_b, vp, top_name, top_prob, bias_summary
                )
            )
        else:
        ...
        if len(scenarios) == 1 and scenarios[0].name.startswith("Wait"):
            ...
            for sc in scenarios:
                ...
                if key == top_name:
                    main_sc = sc
                    main_prob = sc_probs.get(key, 0.0)
                    break
            if main_sc is None:

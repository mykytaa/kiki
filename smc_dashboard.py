...
С поддержкой экспорта сценария в TradingView (Pine v5) и
кнопки “⚡ Нанести на TradingView автоматически” (через Tampermonkey userscript).
"""
...
BINANCE_INTERVAL = {"5m": "5m", "15m": "15m", "1h": "1h"}
HTF_MAP = {"5m": "15m", "15m": "1h", "1h": "4h"}  # HTF для анализа; 4h тянем в режимах
DAILY_LIMIT = 240
LTF_LIMIT = 600
HTF_LIMIT = 400
...
def _tv_symbol(symbol: str) -> str:
    # Подберите под свой источник на TV. Для Binance подойдёт так:
...
def pine_for_scenario(symbol: str, tf: str, sc) -> str:
    """Сгенерировать Pine v5, который рисует Entry/SL/TP1/TP2 и подпись сценария."""
...
def _tv_auto_url(symbol: str, tf: str, sc) -> str:
    """Собираем URL tradingview.com с Pine-кодом в #hash (для Tampermonkey userscript)."""
    pine_code = pine_for_scenario(symbol, tf, sc)
    payload = base64.b64encode(pine_code.encode("utf-8")).decode("ascii")
...
def propose(df: pd.DataFrame, htf_bias: str, d_bias: str, regime: str, vp: Dict[str, float | np.ndarray], obv_slope: float) -> List[Scenario]:
    ...
    near_bsl = sorted([p for _, p in pools["BSL"]], key=lambda x: abs(x - c))[:2]
    near_ssl = sorted([p for _, p in pools["SSL"]], key=lambda x: abs(x - c))[:2]

    sc: List[Scenario] = []

    # Trend continuation (FVG)
    ...
    # Structure Breakout (две стороны при наличии уровней)
    ...
    # Сортировка по режиму и согласованию с HTF
...
# ========= Probabilities (softmax; never 100%) =========
...
def render_beginner_card(st, sc: Scenario, prob: float, atr_val: float, symbol: str, tf: str):
    ...
    st.caption("Для автонанесения установите Tampermonkey userscript (см. инструкцию). Иначе используйте Copy/Download.")
...
def binance_klines(symbol: str, interval: str, limit: int = 800) -> pd.DataFrame:
    ...
    df["open_time"] = to_dt(df.open_time)
...
for s in SYMBOLS:
    try:
        ...
        htf = HTF_MAP[tf]
        # Если HTF=4h не поддержан в BINANCE_INTERVAL для графика — возьмём 1h как суррогат
        ...
        if not [x for x in scenarios if not x.name.startswith("Wait")]:
            scenarios = scenarios_all  # если всё вырезали — вернём исходные
        ...
        ltf_b = score_bias(df)
        htf_b = score_bias(df_h)
        d_b = regime_daily(df_d)
